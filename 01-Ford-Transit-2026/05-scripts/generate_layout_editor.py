#!/usr/bin/env python3
"""
Ford Transit 148" ELWB High Roof – Enhanced Interactive Layout Editor
=====================================================================
Generates a self-contained HTML layout editor with accurate structural
elements modeled after FarOutRide-style 3D reference:

  STRUCTURAL (locked, toggleable layers):
  ├─ Dome-shaped wheel wells (arch profile, not boxes)
  ├─ Window openings with frames (3 port + 2 stbd + rear door glass)
  ├─ Sliding door opening (starboard) with threshold
  ├─ Rear cargo door opening with sill (60/40 split)
  ├─ Fuel tank (underfloor, port/driver side)
  ├─ DEF tank (underfloor, starboard – diesel models, off by default)
  ├─ Floor ribs / corrugations
  ├─ B / C / D structural pillars
  ├─ Roof cross-members / ribs
  ├─ Factory floor tie-down points
  └─ Superellipse parametric roof + walls

  INTERACTIVE FEATURES:
  ├─ Layer toggle panel (show/hide each structural group)
  ├─ View presets (Top, Driver Side, Rear, 3/4)
  ├─ Per-item position / dimension / color sliders
  ├─ Add / duplicate / delete custom items
  ├─ Preset buttons: overhead cabinet, shelf, drawer, L-track
  ├─ Collision / overlap detection warnings
  ├─ Export / import layout JSON
  ├─ Snap-to-grid option (G key)
  └─ Keyboard shortcuts (1-4 views, D duplicate, Del delete)

All dimensions from Ford Transit 148" ELWB High Roof specifications.
Coordinate system: X = width (port→stbd), Y = length (front→rear), Z = height.

Output:  04-outputs/20260220-layout-editor.html
Usage:   python generate_layout_editor.py
"""
...
# ═══════════════════════════════════════════════════════════════════════
# Cargo area constants (inches)  — Adrian Steel Layout Guide, 148" EXT WB
# ═══════════════════════════════════════════════════════════════════════
CL = 172.2   # cargo length (Ford spec: partition face to rear bumper)
CW = 68.0    # cargo width at floor (layout guide; beltline=70.2")
CH = 79.0    # cargo height, high roof (layout guide)

# ═══════════════════════════════════════════════════════════════════════
# Default interior layout items (inches, user-editable)
# ═══════════════════════════════════════════════════════════════════════
...
DEFAULT_ITEMS = [
    ...
    {"id":"sink",         "name":"Z4 – Sink",             "zone":"Zone 4 – Galley",
     "x":50.2,"y":91, "z":29, "dx":15, "dy":13, "dz":7,
    ...
    # Overhead cabinets
    {"id":"upper_bath",   "name":"Z3 – Upper Bath Cabinet","zone":"Overhead Storage",
     "x":0,   "y":70, "z":54, "dx":26, "dy":44, "dz":14,
     "color":"#2E86C1", "opacity":0.35},
    {"id":"upper_galley", "name":"Z4 – Upper Galley Cab", "zone":"Overhead Storage",
     "x":48.2,"y":70, "z":54, "dx":22, "dy":44, "dz":18,
     "color":"#8B6248", "opacity":0.45},

    # Roof fans
    {"id":"fan1",         "name":"Roof Fan 1",            "zone":"Ventilation",
     "x":28.1,"y":42, "z":78.5,"dx":14,"dy":14, "dz":3,
     "color":"#E59866", "opacity":0.85},
    {"id":"fan2",         "name":"Roof Fan 2",            "zone":"Ventilation",
...
def build_html(items_json: str) -> str:
    return (
        _HTML_HEAD
        + _HTML_BODY
        + "<script>\n"
        + f"const DEFAULT_ITEMS_JSON = {items_json};\n"
        + _JS_CODE
        + "\n</script>\n</body>\n</html>"
    )
...
<title>Ford Transit Van Build – Layout Editor v2</title>
...
# ── HTML body (no script yet) ────────────────────────────────────────

_HTML_BODY = r"""
<body>
<div id="sidebar">
  <div class="panel-header">
    <h2>🚐 Ford Transit Layout Editor v2</h2>
    <p>148" ELWB High Roof · All dims in inches · Structural from Ford specs</p>
  </div>
  <div id="tabs">
    <div class="tab active" onclick="switchTab('layout')">🛋️ Layout</div>
    <div class="tab" onclick="switchTab('structure')">🏗️ Structure</div>
    <div class="tab" onclick="switchTab('info')">📐 Info</div>
  </div>
  <div id="toolbar">
    <button class="primary" onclick="addItem()">＋ Add</button>
    <button onclick="addPreset('overhead_cab')">＋ Cabinet</button>
    <button onclick="addPreset('shelf')">＋ Shelf</button>
    <button onclick="addPreset('drawer')">＋ Drawer</button>
    <button onclick="addPreset('ltrack')">＋ L-Track</button>
  ...
  <div class="tab-panel" id="tab-info">
    <div style="padding:8px;font-size:11px;line-height:1.7;color:var(--muted);">
      ...
      <table style="width:100%;border-collapse:collapse;font-size:10px;">
        <tr><td style="padding:3px 6px;color:var(--text);">Cargo Length (Ford spec)</td><td><b>172.2"</b> (partition → bumper)</td></tr>
        <tr><td style="padding:3px 6px;color:var(--text);">Usable Floor Length</td><td><b>145"</b> (partition → rear door)</td></tr>
        ...
        <tr><td style="padding:3px 6px;color:var(--text);">Sliding Door Opening</td><td><b>51"W × 67"H</b> (stbd)</td></tr>
        <tr><td style="padding:3px 6px;color:var(--text);">Rear Door Opening</td><td><b>62"W × 65"H</b> (60/40)</td></tr>
        ...
        <tr><td style="padding:3px 6px;color:var(--text);">Wheel Well Zone</td><td><b>68–103"</b> from partition</td></tr>
        <tr><td style="padding:3px 6px;color:var(--text);">Behind Wheel Wells → Rear</td><td><b>42"</b> available space</td></tr>
      ...
      <div><b>1–6</b>: View presets (3/4, Top, Driver, Pass, Rear, Front)</div>
      ...
      <div>• ⛽ Fuel tank shown below Z=0 — check clearance for underfloor plumbing</div>
      <div>• 🔶 Wheel well arches show true dome — test furniture fit at curves</div>
      <div>• 🚪 Sliding door zone (passenger side, Y=0–51") keep clear for entry</div>
      <div>• Roof ribs spaced ~22" — mount overhead cabinets to ribs for strength</div>
      <div>• Toggle layers in Structure tab to reduce visual clutter</div>
      <div>• Floor tie-down points (📌) show L-track / anchor bolt locations</div>
    </div>
  </div>
</div>
...
<div id="viewport">
  <div id="view-bar">
    ...
    <button id="snap-btn" onclick="toggleSnap()" style="opacity:0.5;">⊞ Snap</button>
  </div>
...
<div class="modal-bg" id="add-modal">
  <div class="modal">
    ...
    <select id="new-zone">
      <option>Zone 1 – Sofa Bed</option><option>Zone 2 – Bed</option>
      <option>Zone 3 – Wet Bath</option><option>Zone 4 – Galley</option>
      <option>Zone 5 – Aisle</option><option selected>Overhead Storage</option>
...
// Upfit Supply PDF – Ford Transit 148" EXT WB (SRW):
// Floor=145", WW=35" long, partition→WW rear=103", behind WW=42"
// WW front = 103−35 = 68", WW rear = 103"
...
// Cross-section = wall face + quarter-ellipse arch + floor — convex shape
function mkWheelWell(side){
  ...
  return{x:xs,y:ys,z:zs}; // no i,j,k — use alphahull:0
}
...
function buildStructural(){
  ...
  // ── Walls ──
  if(layers.walls.on){
    T.push(m3d(mkBox(-3,-4,0,3,CL+8,CH),'#667788',0.05,'Wall – Stbd (Passenger)','Walls',true));
    T.push(m3d(mkBox(CW,-4,0,3,CL+8,CH),'#667788',0.05,'Wall – Port (Driver)','Walls'));
  ...
  if(layers.wheelWells.on){
    ...
    for(const [side,nm,sl] of [['port','Wheel Well – Passenger',true],['stbd','Wheel Well – Driver',false]]){
  ...
  if(layers.windows.on){
    ...
    // Passenger (stbd) – 2 windows (sliding door takes one slot) – now on x=0 side
    const pW=[[55,85],[96,132]];
    for(const [y0,y1] of pW){
      T.push(m3d(mkBox(-2,y0,sill,1.5,y1-y0,wH),'#5DADE2',0.22,'Window–Passenger','Windows'));
      const fx=-2.5,fw=0.5;
      T.push(m3d(mkBox(fx,y0-1,sill-1,fw,y1-y0+2,1),'#444',0.55,'Frame','Windows'));
      T.push(m3d(mkBox(fx,y0-1,sill+wH,fw,y1-y0+2,1),'#444',0.55,'Frame','Windows'));
      T.push(m3d(mkBox(fx,y0-1,sill,fw,1,wH),'#444',0.55,'Frame','Windows'));
    ...
    // Sliding door window – now on x=0 (passenger) side
  ...
  if(layers.slidingDoor.on){
    ...
    T.push({type:'scatter3d',x:[-5],y:[dy/2],z:[dh+3],mode:'text',
      text:['SLIDING DOOR (51"W × 67"H) ►'],textfont:{size:9,color:'#2ECC71'},
      showlegend:false,hoverinfo:'skip'});
  }
  ...
  if(layers.rearDoors.on){
    ...
    T.push({type:'scatter3d',x:[CW/2],y:[CL+6],z:[rdH+4],mode:'text',
      text:['REAR DOORS (60/40) 62"W × 65"H'],textfont:{size:9,color:'#E74C3C'},
      showlegend:false,hoverinfo:'skip'});
  }
  ...
  if(layers.fuelTank.on){
    ...
    T.push({type:'scatter3d',x:[CW-12],y:[28],z:[-14],mode:'text',
      text:['⛽ FUEL TANK'],textfont:{size:8,color:'#E67E22'},showlegend:false,hoverinfo:'skip'});
  ...
  if(layers.tieDowns.on){
    ...
    for(let i=0;i<pts.length;i++){
      ...
      T.push({type:'scatter3d',x:[px],y:[py],z:[0.6],mode:'markers',
        ...
        showlegend:false,hoverinfo:'name',name:'📌 Tie-Down'});
    }
  }
  ...
  if(layers.dims.on){
    T.push({type:'scatter3d',x:[CW/2,CW/2],y:[-6,-22],z:[0,0],
      mode:'lines+text',text:['','◄── FRONT (CAB)'],textposition:'bottom center',
...
function itemTraces(){
  ...
  for(const it of items){
    ...
    T.push({type:'mesh3d',x:b.x,y:b.y,z:b.z,i:b.i,j:b.j,k:b.k,
      ...
      hovertext:it.name+'\n'+it.dx.toFixed(1)+'W × '+it.dy.toFixed(1)+'D × '+it.dz.toFixed(1)+'H'+
...
const PLOTLY_LAYOUT={
  scene:{
    xaxis:{title:'Width (pass→driver) [in]',range:[-30,CW+30],
...
function render(){
  ...
  document.getElementById('warnings').textContent=w.length?'⚠ '+w.join(' · '):'';
  document.getElementById('status-dims').textContent=items.length+' items · '+CL+'L × '+CW+'W × '+CH+'H in';
}
  ...
  for(const it of items){
    ...
    if(!locked){
      b.innerHTML='<div class="prop-group-label">Position</div>'+
        ...
        '<div class="prop-row"><span class="prop-label">🎨</span>'+
        ...
        '<div class="item-actions"><button onclick="dupItem(\''+it.id+'\')">⧉ Dup</button><button class="del" onclick="delItem(\''+it.id+'\')">✕ Del</button></div>';
    } else {
      b.innerHTML='<div style="font-size:10px;color:#484f58;padding:4px;">🔒 Locked</div>';
    }
...
function confirmImport(){try{
  ...
  document.getElementById('status-text').textContent='✅ Imported!';
...
def main():
    print("=" * 64)
    print("  Ford Transit Van Build – Enhanced Layout Editor Generator")
    print("=" * 64)

    items_json = json.dumps(DEFAULT_ITEMS)
    html = build_html(items_json)
    ...
    print("    🔶 Dome-shaped wheel wells (arch profile)")
    print("    🪟 Windows with frames (3 port, 2 stbd + sliding door + rear glass)")
    print("    🚪 Sliding door opening (stbd) with threshold & step")
    print("    🚪 Rear cargo doors (60/40 split) with sill")
    print("    ⛽ Fuel tank (underfloor, driver side) with floor projection")
    print("    💧 DEF tank (underfloor, diesel – off by default)")
    print("    ═══ Floor ribs / corrugations (lateral + longitudinal)")
    print("    ┃  B / C / D structural pillars (6 total)")
    print("    ─── Roof cross-members (8 ribs @ ~22\" spacing)")
    print("    📌 Factory floor tie-down points (8 locations)")
    print()
    print("  Features:")
    print("    🏗️  Structure tab – toggle each layer group on/off")
    print("    📐 Info tab – dimensions chart + keyboard shortcuts")
    print("    👁️  6 view presets (keys 1–6)")
    print("    ⊞  Snap-to-grid (1\" grid, G key)")
    print("    ⚠️  Collision warnings (walls, ceiling, wheel wells, sliding door)")
    print("    ⌨️  Keyboard shortcuts (views, D=dup, Del=delete, Esc=deselect)")
    print("    📦 Presets: Cabinet, Shelf, Drawer, L-Track")
